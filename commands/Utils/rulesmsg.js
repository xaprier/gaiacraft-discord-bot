    ...
    run: async (client, message, args) => {
        ...
        if (user.permissions.has("ADMINISTRATOR")) {
            ...
            embed.setDescription(``).addFields(
                { name: `İsim/Açıklama/Hakkında`, value: `Discord hesabınızın **İsim/Açıklama/Hakkında** bölümlerinde reklam, uygunsuz içerik olması yasaktır. Ceza hakkında bilgi almak için <#${config.punishChannel}> kanalını görüntüleyebilirsiniz.` },
                { name: `Destek`, value: `Unutmayın ki yetkililer de insan, bu yüzden onları sunucu kapanma gibi olaylar dışında taleplerde etiketlemeyiniz. Yetkililer müsait ise zaten en kısa sürede talebe dönüş yapacaklardır. Ceza hakkında bilgi almak için <#${config.punishChannel}> kanalını görüntüleyebilirsiniz.
                `},
                { name: `Sohbet`, value: `Sohbeti sorunlarınızla ve kavga ile kirletmek, her türlü küfür/hakaret ve diğer sohbet kurallarını çiğnemek yasaktır. Ceza hakkında bilgi almak için <#${config.punishChannel}> kanalını görüntüleyebilirsiniz.`}
            ).author.name = `Discord`;
            try {
                ruleChannel.send({ embeds: [embed] });
                message.delete();
